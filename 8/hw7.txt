...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    same_x = real_close(@x1,@x2)
    same_y = real_close(@y1,@y2)
    if real_close_point(@x1,@y1,@x2,@y2)
      Point.new(@x1,@y1)
    elsif (@x1 > @x2 and not same_x) or (@y1 > @y2 and not same_y)
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1,@x2) # test for vertical line
      if @y1 < seg.y1
...
class Var < GeometryExpression
  ...
  def eval_prog env 
    var_assignment = env.reverse.find { |v| v.first == @s }
    var_assignment ? var_assignment.last : raise("var not found: #{@s}")
  end
  def preprocess_prog
    self
  end
